...
void draw() {
  ...
  for (int i = 0; i< blobMatrix.size(); i++) {
    ...
    PVector [] bb = sm.getOOBB(convexHull); //get the corner points of the min bounding box
    //draw the minimum bounding box
    strokeWeight(3);
    stroke(0,0,255);
    beginShape();
    vertex(bb[0].x,bb[0].y);
    vertex(bb[1].x,bb[1].y);
    vertex(bb[2].x,bb[2].y);
    vertex(bb[3].x,bb[3].y);
    endShape(CLOSE);
 }
}